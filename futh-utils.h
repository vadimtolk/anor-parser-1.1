...
class TestUser {
	...
	public:
		...
		const std::pair<const uint32_t, char*> calculateAnor() noexcept {
			try {
				std::cout << "|---> ANOR-parser 1.2 by Futhex Core. <---|\n\n";
				...
				// WHILE TEXT VALIDATION NOT CREATED!!! (indexes 0 - 4)
				uint64_t falloc_mbytes = *(lexed + 0);
				uint64_t valloc_mbytes = *(lexed + 1);

				if (!falloc_mbytes | !valloc_mbytes) throw std::invalid_argument("Cannot allocate 0 MB (someone field wasn't declarated\n");
